...
from vias.utils.helpers import MapInfo, ScenarioInfo
...
def create_test_folder():
    ...
    if os.path.exists(test_folder_path):
        assert False, "Delete old test folder before creating a new one"
...
def delete_test_folder():
    ...
    if not os.path.exists(test_folder_path):
        assert False, "Create test folder path before deleting it"
...
class TestSimulatorGridBased(unittest.TestCase):
    ...
    def create_test_scenario(self):
        ...
        scenario = Scenario(self.scenario_info)
