...
from vias.utils.helpers import MapInfo, get_tmerc_map_origin, TmercCoord, LocalCoord, ArrayCoord, OutOfOperationSpace, \
    ScenarioInfo, get_map_identifier, get_graph_identifier
...
def get_local_to_tmerc_transform(map_info: MapInfo, tmerc_projection: Optional[Proj] = None) -> np.ndarray:
    tmerc_map_origin = get_tmerc_map_origin(map_info, tmerc_projection=tmerc_projection)
    T_trans = np.array(
        [[1, 0, tmerc_map_origin.east], [0, 1, tmerc_map_origin.north - map_info.y_length], [0, 0, 1]])
    T_tot = T_trans
    return T_tot


def get_tmerc_to_local_transform(map_info: MapInfo, tmerc_projection: Optional[Proj] = None) -> np.ndarray:
    return np.linalg.inv(get_local_to_tmerc_transform(map_info, tmerc_projection=tmerc_projection))


def local_coord_from_tmerc_coord(tmerc_coord: TmercCoord, map_info: MapInfo, height_val=0.0,
                                 transform: Optional[ndarray] = None) -> LocalCoord:
    if transform is None:
        transform = get_tmerc_to_local_transform(map_info)
    local_array = np.matmul(transform, np.array([[tmerc_coord.east], [tmerc_coord.north], [1]]))
    return LocalCoord(local_array[0].item(), local_array[1].item(), height_val)


def tmerc_coord_from_local_coord(local_coord: LocalCoord, map_info: MapInfo) -> TmercCoord:
    transform = get_local_to_tmerc_transform(map_info)
    tmerc_array = np.matmul(transform, np.array([[local_coord.x], [local_coord.y], [1]]))
    return TmercCoord(tmerc_array[0].item(), tmerc_array[1].item())


class GridGraph(object):
    def __init__(self, map_info: MapInfo, dimension=3):
    ...
    @property
    def directed(self) -> Optional[bool]:
        if self.graph is not None:
            return self.graph.is_directed()
        else:
            return None

    def local_coord_from_tmerc_coord(self, tmerc_coord: TmercCoord, height_val=0.0) -> LocalCoord:
        inverse_transform = self.inverse_grid_map_transform
        local_array = np.matmul(inverse_transform, np.array([[tmerc_coord.east], [tmerc_coord.north], [1]]))
        return LocalCoord(local_array[0].item(), local_array[1].item(), height_val)
    ...
    def local_coord_from_array_coord(self, array_coord: ArrayCoord) -> LocalCoord:
        ...
        local_array = np.matmul(transform,
                                np.array([[array_coord.row], [array_coord.col], [array_coord.lay], [1]])).astype(float)
        return LocalCoord(local_array[0].item(), local_array[1].item(), local_array[2].item())

    def local_coord_lies_in_map(self, local_coord: LocalCoord) -> bool:
        if local_coord.x < 0 or local_coord.y < 0 or local_coord.z < 0:
            return False
        if local_coord.x >= self.x_length or local_coord.y >= self.y_length or local_coord.z >= self.z_length:
            return False
        return True
    ...
    def array_coord_from_local_coord(self, local_coord: LocalCoord) -> ArrayCoord:
        if not self.local_coord_lies_in_map(local_coord):
            raise OutOfOperationSpace("array_coord_from_local_coord()",
                                      f"Local coord {local_coord} lies outside of operation space.")
        inverse_transform = self.inverse_array_transform
        array_array = np.matmul(inverse_transform, np.array([[local_coord.x], [local_coord.y], [local_coord.z], [1]]))
        return ArrayCoord(int(np.ceil(array_array[0]).item()), int(np.floor(array_array[1]).item()),
                          int(np.floor(array_array[2]).item()))

    def array_coords_from_local_coords(self, local_coords: List[LocalCoord]) -> List[ArrayCoord]:
        for local_coord in local_coords:
            if not self.local_coord_lies_in_map(local_coord):
                raise OutOfOperationSpace("array_coord_from_local_coord()",
                                          f"Local coord {local_coord} lies outside of operation space.")
        inverse_transform = self.inverse_array_transform
        row_vectors = np.vstack([local_coord.as_homogeneous() for local_coord in local_coords])
    ...
    def local_coords_from_array_coords(self, array_coords: List[ArrayCoord]) -> List[LocalCoord]:
        transform = self.array_transform
        row_vectors = np.vstack([array_coord.as_homogeneous() for array_coord in array_coords])
    ...
    def array_transform(self):
        """Calculates a transform
            ...
            whereas x, y and z are the Local coordinates in meter with its coordinate frame located in the lower left;
            x pointing to the right, and y pointing upwards and z pointing out of the plain;

            row, col and lay are the respective numpy array indices with the origin in the upper left.
    ...
    def grid_map_transform(self):
        """Calculates an affine transform
            ...
            whereas east and north are the spatial (e.g. tmerc) coordinates east pointing to the right and north pointing
            upwards;

            and x and y are the cartesian coordinates with its coordinate frame located in the lower left;
            x pointing to the right, and y pointing upwards.
    ...
    def inverse_grid_map_transform(self):
        ...
        if self._inverse_grid_map_transform is None:
            self._inverse_grid_map_transform = get_tmerc_to_local_transform(self.map_info)
    ...
    def plot_paths(self, paths: Union[Path, List[Path]], savepath='', suffix='', title='', prefix="", linestyle='', markerstyle='.', plot_lib="plotly"):
        pass
...
def save_grid_graph(grid_graph: GridGraph, data_save_folder: str, scenario_info: ScenarioInfo,
                    grid_graph_name: str) -> None:
    """
    By default the grid_map is saved in the "grid_maps" folder within the data_save_folder
    """
...
def load_grid_graph(data_save_folder: str, scenario_info: ScenarioInfo, grid_graph_name: str) -> Union[GridGraph, 'GridMap']:
    grid_map_path = get_grid_graph_path(data_save_folder, scenario_info, grid_graph_name)
    if os.path.exists(grid_map_path):
        with open(grid_map_path, 'rb') as f:
            grid_map = pickle.load(f)
        return grid_map
    else:
        assert False, f"Cannot load from path {grid_map_path}"
...
def get_grid_graph_path(data_save_folder, scenario_info, grid_graph_name) -> str:
    ...
    if "graph" in grid_graph_name:
        assert "map" not in grid_graph_name, "The word map and graph are both in the name of the, avoid this."
        return os.path.join(data_save_folder, "grid_graphs",
                            f"{get_graph_identifier(scenario_info)}_{grid_graph_name}.pkl")
    elif "map" in grid_graph_name:
        assert "graph" not in grid_graph_name, "The word map and graph are both in the name of the, avoid this."
        config = get_config()
        map_dim = config.get(grid_graph_name).dimension
        return os.path.join(data_save_folder, "grid_maps",
                            f"{get_map_identifier(scenario_info, map_dim)}_{grid_graph_name}.pkl")
    else:
        assert False, f"Unknown name tag {grid_graph_name}, either the word map or the word graph must be in the name."
