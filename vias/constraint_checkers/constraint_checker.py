...
class ConstraintChecker(ABC):
    """This is an abstract class (~interface) for any ConstraintChecker (Constraint Simulator) that is implemented in the framework. Every conulator
    ...
    def load_inputs(self, inputs_str: List[str], input_data_folder: str):
        raise NotImplementedError('Users must define load_inputs() to use this base class.')
    ...
    def check_constraint(self, path: Path) -> Tuple[float, Constraint]:
        """Starts the conulation. The inputs for the conulation can be found in self.input. After the conulation
        its outputs should be written into self.quality_criteria. This is then returned by the method."""
        raise NotImplementedError('Users must define check_constraint to use this base class.')
