...
class ParetoHandler:
    """This is an class for managing data of multi-objective based optimizers.
    Saving individual's fitness per
    ...
    def insert_F_X_dict(self, F_X_dict: dict):
        assert self.is_empty(), (
            "ParetoHandler needs to be empty in order to load " "F_X_data"
        )

        objective_function_vals = list(F_X_dict.keys())
        individuals = list(F_X_dict.values())
        objective_function_values: np.ndarray = np.array(
            [np.array(el) for el in objective_function_vals]
        )
        ...
        non_dominated_indices = self.non_dom_sort.do(
            objective_values, only_non_dominated_front=True
        )
    ...
    def hyp_vol(self):
        if self._hyp_vol is None:
            assert len(self.data["F"]) > 0, (
                "hypervolume calculation was triggered before "
                "first individuals are known"
            )
            reference_point = np.max(self.data["F"][-1], axis=0) * 1.2
            self._hyp_vol = Hypervolume(
                ref_point=reference_point, pf=self.data["F"][0], nds=False
            )
    ...
    def init(self, path_to_init):
        if path_to_init == "":
            ...
            assert isinstance(
                self.data, dict
            ), "Input object is expected to be a dictionary"
            assert len(self.data.keys()) == len(
                self.expected_columns
            ), f"Input dictionary expected to have {len(self.expected_columns)} keys."
            assert set(self.data.keys()) == set(
                self.expected_columns
            ), f"Input dictionary is expected to have the keys {self.expected_columns}."

    def update_statistic(self, generation_count: int, pop: Population, n_eval: int):
        """Calculates important metrics of a whole population that are minimum achieved
        fitness, maximum achieved fitness, mean and standard deviation of all
        fitness values of one population."""
    ...
    def get_extreme_solution(self, idx_objective) -> tuple[float, Individual]:
        """Return the extreme objective function value given a objective
        index and the respective individual"""
        ...
        return non_dom_objective_function_values[extr_idx][
            idx_objective
        ], non_dom_individuals[extr_idx]
    ...
    def _get_knee_point_index(self):
        ...
        normalized_data = (normalized_data - np.min(normalized_data, axis=1)) / (
            np.max(normalized_data, axis=1) - np.min(normalized_data, axis=1)
        )
    ...
    def get_F_X_dict(self):
        return dict(
            zip(
                [tuple(row) for row in self.data["F"][-1]],
                self.data["X"][-1],
                strict=False,
            )
        )

    def get_objective_function_values(self, obj_idx: list | None = None):
        # returns the current objective function values (can also include
        # dominated solutions)
        if obj_idx is None:
    ...
    def get_pareto_solutions(self):
        # returns the solutions (individuals) for all non-dominated objective
        # function values
    ...
    def print_statistics(self):
        ...
        if not output_redirectd:
            columns = [
                self.data[header][-10:] for header in headers
            ]  # the latest 10 rows
        else:
    ...
    def save_and_clean(self):
        ...
        with open(
            os.path.join(DataManager().data_output_path, "F_X_dict.pkl"), "wb"
        ) as handle:
            pickle.dump(self.get_F_X_dict(), handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(
            os.path.join(DataManager().data_output_path, "optimizer_statistics.pkl"),
            "wb",
        ) as handle:
            pickle.dump(self.data, handle, protocol=pickle.HIGHEST_PROTOCOL)

    def save_generation_statistic_plot(self, show_figure=False):
        """This method can be used during each iteration of the optimizer to save an
        updated version of the result's figure plot.
        """
        config = get_config()
        data = self.data
        n_gen = data["n_gen"]
        ...
        # plot HV
        fig.add_trace(
            go.Scatter(x=n_gen, y=data["HV"], mode="lines", name="Hypervolume"),
            secondary_y=False,
        )
        fig.add_trace(
            go.Scatter(x=n_gen, y=data["delta_hv"], mode="lines", name="delta_hv"),
            secondary_y=True,
        )
        fig.add_trace(
            go.Scatter(
                x=n_gen, y=data["delta_f"], mode="lines", name="delta_f/current IGD"
            ),
            secondary_y=True,
        )
        fig.add_trace(
            go.Scatter(
                x=n_gen, y=data["delta_nadir"], mode="lines", name="delta_nadir"
            ),
            secondary_y=True,
        )
        fig.add_trace(
            go.Scatter(
                x=n_gen, y=data["delta_ideal"], mode="lines", name="delta_ideal"
            ),
            secondary_y=True,
        )
        fig.add_trace(
            go.Scatter(
                x=n_gen, y=data["n_cv"], mode="lines", name="# constraint violations"
            ),
            secondary_y=True,
        )
        for i in range(0, data["nadir"][-1].size):
            fig.add_trace(
                go.Scatter(
                    x=n_gen,
                    y=[el[i] for el in data["nadir"]],
                    mode="lines",
                    name=f"Nadir axis {i + 1}",
                ),
                secondary_y=False,
            )
            fig.add_trace(
                go.Scatter(
                    x=n_gen,
                    y=[el[i] for el in data["ideal"]],
                    mode="lines",
                    name=f"Ideal axis {i + 1}",
                ),
                secondary_y=True,
            )

        fig.update_layout(title=dict(text="Optimization Callback Data"))
