...
class Optimizer(ABC):
    """This is an abstract class (~interface) for any evolutionary Optimizer that is
    implemented in the framework. Every optimizer
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.evaluator: Evaluator = kwargs["evaluator"]
    ...
    def init_optimizer(self):
        """This method is called upon initialization of the optimizer"""
        raise NotImplementedError(
            "Users must define init_optimizer to use this base class."
        )

    @abstractmethod
    def optimize(self):
        """This method starts the optimizer. It should run for self.iterations
        iterations. The input of the optimizer
        can be found in self.coder.encoding_result. Objectives are given in
        self.objectives. During the execution the optimizer has to call
        self.evaluate() in order to get criteria values for the current result.
        Statistics can be saved by the help
        of self.add_statistic_line() and self.save_generation_statistic_plot(). At
        the end of this method the
    ...
    def finish_after_optimization(self):
        ...
        raise NotImplementedError(
            "Users must define clean_after_optimization to use this base class."
        )

    @abstractmethod
    def close_optimization(self):
        raise NotImplementedError(
            "Users must define close_optimization to use this base class."
        )

    def evaluate(self, individual: np.ndarray):
        """This method will be called in the optimize() method. After the optimizer
        has provided an intermediate result/
        solution one or more quality criteria are calculated here, dependent on how
        many simulators had been added
        to the evaluator by the user. The provided code is ready to use but can be
        extended by quality/constraint
        checks for example."""
    ...
    def evaluate_parallel(self, pop):
        """Does the same like the evaluate() function but supposed to work in a
        parallel manner through multiple
        processes."""
        config = get_config()
        ...
        for w in range(num_processes):
            p = Process(
                target=self.evaluate_single_ind,
                args=(individuals_to_process, individuals_processed, w),
            )
            processes.append(p)
            p.start()
        ...
        assert len(ind_fit_con_tuple) == before_qsize, (
            "Not all individuals had been processed, there is probabily"
            "something wrong with the multiprocessing."
        )
        ind_fit_con_tuple.sort()  # sorts indices of individuals so that the order
        # that may be disturbed due to multiprocessing is like before
    ...
    def evaluate_single_ind(self, individuals_to_process, individuals_processed, idx):
        raise NotImplementedError(
            "Users must define evaluate_single_ind to use this base class."
        )
    ...
    def get_num_unsatisfied_niches(self, pop):
        ...
        for niche_id, niche in enumerate(pop.get_niches()):
            if niche_id == 0:
                ...
                continue
                # this is the niche, where all the straight line paths live,
                # we won't expect them to satisfy all constraints,
                # therefore we skip them
            niche_satisfied = False
            for ind in niche:
                assert len(ind.constraints) > 0, (
                    "Constraint satisfaction stop "
                    "criterion was chosen, but there "
                    "are no constraints"
                )
                all_constraints_satisfied = True
                for constraint in ind.constraints:
                    if not constraint.satisfied:
                        # console.log(
                        #     f"Constraint {constraint.name} with value {
                        #     constraint.value} in
                        #     niche {niche_id} is not satisfied.")
        ...
        if num_unsatisfied_niches == 0:
            ...
            console.log(
                f"{num_unsatisfied_niches} are still unsatisfied. Go on with niches."
            )
