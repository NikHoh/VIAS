...
from deap.tools import initRepeat, mutGaussian, selNSGA2, selNSGA3WithMemory
from deap.tools.crossover import cxOnePointDeterminedLength
from deap.tools.emo import selTournamentConstraint, selTournamentDCD
from deap.tools.selection import selBest, selTournament
...
from vias.optimizers.optimizer import OptimizationEndsInNiches, Optimizer
from vias.optimizers.optimizer_helpers import Individual, Population
from vias.path import Path
from vias.utils.tools import bcolors as bc


class OptimizerGA(Optimizer):
    """This is an implementation of the abstract Optimizer class. It describes an
    genetic algorithm (GA), whose
    individuals are floating point lists ([a,b,c,d,e,f,...]). The selection strategy
    is based on DEBs NSGAII selection
     for biobjective optimization problems, thus giving it the same NSGA2 (
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tracked_termination_value: float | None = np.nan
        ...
        if self.enable_niching:
            assert config.preprocessor.advanced_individual_init, (
                "Niching makes only " "sense with " "precomputed good " "solutions"
            )
    ...
    def optimize(self):
        ...
        if self.num_objectives == 2:
            # This is just to assign the crowding distance to the individuals (needed
            # for Tournament)
            # no actual selection is done
            pop.set_niches(
                [toolbox.select(niche, len(niche)) for niche in pop.get_niches()]
            )
        ...
        self.pareto_handler.update_statistic(
            self.generation_count, pop, self.eval_count
        )
        ...
        while not self.termination_criterion_valid():
            ...
            offspring.set_niches(
                [
                    self.var_individual(
                        niche, toolbox, len(niche), self.prob_cross, self.prob_mut
                    )
                    for niche in pop.get_niches()
                ]
            )
            ...
            pop.set_niches(
                [
                    toolbox.select(pop_niche + offspring_niche, len(pop_niche))
                    for pop_niche, offspring_niche in zip(
                        pop.get_niches(), offspring.get_niches(), strict=False
                    )
                ]
            )
            ...
            # save network representations for all time best individuals with respect
            # to every single objective

            if not config.suppress_iteration_based_paths_plot:
                for idx, objective in enumerate(self.objectives):
                    extreme_individual_fitness, extreme_individual = (
                        self.pareto_handler.get_extreme_solution(idx)
                    )
                    self.plot_extreme_path(
                        extreme_individual, extreme_individual_fitness, objective
                    )
                    # check extreme individuals for constraints
                    self.check_extreme_solution_constraints(
                        extreme_individual, objective
                    )
    ...
    def check_extreme_solution_constraints(self, extreme_individual, objective):
        for constraint in extreme_individual.constraints:
            if not constraint.satisfied:
                console.log(
                    bc.FAIL + f"Critical {constraint.name} constraint was not "
                    f"satisfied (penalty: {constraint.value}) for best "
                    f"solution regarding {objective} objective." + bc.ENDC
                )

    def plot_extreme_path(
        self, extreme_individual, extreme_individual_fitness, objective
    ):
        config = get_config()
        extreme_path = self.coder.decode(extreme_individual)
        path_plot_path_per_objective = os.path.join(
            DataManager().optimized_paths_path, objective
        )
        ...
        title = (
            f"Obj: {objective} Fit:"
            f"{str(round(extreme_individual_fitness, 1))} Gen: {self.generation_count}"
        )
        grid.plot_paths(
            extreme_path,
            savepath=path_plot_path_per_objective,
            suffix=suffix,
            prefix="2D",
            title=title,
        )
    ...
    def close_optimization(self):
        ...
        config = get_config()
        if self.enable_niching:
            raise OptimizationEndsInNiches(
                "optimizer_ga()",
                "End of iterations although population was "
                "still searching for constraint-satisfying "
                "solutions in some niches.",
            )
        for i, objective in enumerate(self.objectives):
            ...
            # plot paths
            title = (
                f"{self.name}'s best found path for {objective} with fitness "
                f"{str(round(fitness, 1))}"
            )
            suffix = f"_{objective}"
            grid.plot_paths(
                extreme_path,
                savepath=DataManager().optimized_paths_path,
                suffix=suffix,
                title=title,
                prefix="2D",
            )
    ...
    def termination_criterion_valid(self):
        if 0 <= self.num_iterations <= self.generation_count:
            console.log(
                "Finished optimization due to 'set number of iterations reached'"
            )
            return True

        if self.num_objectives == 1:  # evaluate stop criterion considering
            # fitness that is not improving
    ...
    def update_niching(self, pop):
        ...
        if num_unsat_niches == 0:
            ...
            num_unsat_niches == 1
            or num_unsat_niches <= int(0.1 * num_niches)
            and int(self.eval_count / num_niches) >= 5500
        ):  # additional quit criterion: if
            # only one single (nasty) unsatisfied niche is left
            self.unsatisfied_iteration_count += 1
        if self.unsatisfied_iteration_count >= 10:  # if additional quit
            # criteria have been valid 10 times, force quit niching
            console.log(
                "Warning: one niche is not satisfying a constraint for 10 iterations "
                "in a row as only nicht. Quit niching."
            )
            self.enable_niching = False
            pop.disable_niching()
            console.log(
                f"Force disabling niching in iteration {self.generation_count}."
            )
    ...
    def evaluate_individuals(self, individuals: list[Individual], toolbox):
        ...
        if len(individuals) > 0:
            if self.multiprocessing_enabled:
                ...
                fitnesses, constraints = zip(
                    *map(toolbox.evaluate, individuals), strict=False
                )

            for ind, fit, constraint in zip(
                individuals, fitnesses, constraints, strict=False
            ):
    ...
    def init_pop(self, toolbox):
        if self.pop_size < 10 * self.num_initial_paths:
            console.log(
                bc.WARNING + f"There are {self.num_initial_paths} pre-processed "
                f"solutions (incl. straight li"
                f"ne) but only {self.pop_size} individuals. This is "
                f"{self.pop_size / self.num_initial_paths} per niche. We "
                f"recommend at least 10 individuals per niche." + bc.ENDC
            )
        individuals_without_noise, niche_numbers = zip(
            *[toolbox.first_ind() for _ in range(self.num_initial_paths)], strict=False
        )
        pop = Population(
            individuals_without_noise, niche_numbers, self.enable_niching
        )  # first individual_germ_cells won't be
        # applied by noise
        if len(pop) < self.pop_size:
            individuals_with_noise, niche_numbers = zip(
                *[
                    toolbox.individual()
                    for _ in range(self.pop_size - self.num_initial_paths)
                ],
                strict=False,
            )
            pop.extend(
                individuals_with_noise, niche_numbers=niche_numbers
            )  # apply noise to the rest
        return pop
    ...
    def finish_after_optimization(self):
        ...
        grid_map.plot_3D_paths(
            ...
            prefix="3D_non_dominated_paths_",
        )

    def init_genetic_algorithm(self):
        """In this method the NSGA2 is initialized, meaning the form of an individual
        is defined, the mutation and
        selection operators are chosen and the objectives are derived."""

        config = get_config()
        # Fitness class and representation of individual
        weights = tuple(
            [-1 for _ in self.objectives]
        ...
        if self.num_objectives == 1:
            ...
            console.log(
                f"The number of reference directions {num_ref_dirs} for the NSGA3 "
                f"selection is automatically chosen to be 1/3 of the population size."
            )
            ref_dirs = get_reference_directions(
                "energy", self.num_objectives, num_ref_dirs, seed=config.seed
            )
            toolbox.register("select", selNSGA3WithMemory(ref_dirs))

        # register the crossover operator
        # toolbox.register("mate", cxSimulatedBinaryBounded, eta=eta_cross,
        # low=self.low_bound, up=self.up_bound)
        toolbox.register(
            "mate", cxOnePointDeterminedLength, cut_length=self.coder.size_gene_per_cp
        )

        # register a mutation operator
        # toolbox.register("mutate", mutPolynomialBounded, eta=eta_mut,
        # low=self.low_bound, up=self.up_bound,
        ...
        toolbox.register(
            "mutate", mutGaussian, mu=mu, sigma=sigma, indpb=self.prob_ind_gene_mut
        )

        return toolbox

    def evaluate_single_ind(self, individuals_to_process, individuals_processed, idx):
        """This function is only called by the evaluate_parallel() function of the
        super class and is called by every
        single evaluation (multiprocessing) process there. Individuals that still
        have to be processed and those that
        are already processed (means that there fitness has been calculated) are
        stored in queues."""
        while True:
            try:
                ...
                individuals_processed.put(
                    (ind_id, tuple(val), constraints)
                )  # caution --> the parameter
                # constraints has not been tested
    ...
    def var_individual(self, population, toolbox, N, cxpb, mutpb):
        ...
        # through niching there is the possibility of an uneven number of individuals
        # in a niche:
    ...
    def generate_individual(
        self, icls, initial_paths: list[Path], weights, apply_noise
    ):
        """Used to initialize an individual which consists of a list of genes.
        Currently there are two nurbs curve
        control points per link in the list of genes, so that the list looks like
        [x_00, y_00, x_01, y_01, x_10, y_10, x_11, y_11, ...]. The initial position
        lies on the line between the NURBS
        curves' end points and are randomly varied a bit. Through the flags
        "use_z_component" and "adaptive_weights"
        ...
        self.initial_path_selection_idx = (
            self.initial_path_selection_idx + 1
        ) % self.num_initial_paths
        ...
        if apply_noise:
            ...
            control_points_array_with_noise = control_points_array + np.random.normal(
                0, np.array([x_std, y_std, z_std]), size=control_points_array.shape
            )
            control_points_with_noise = [
                row.tolist() for row in control_points_array_with_noise
            ]
    ...
    def select_best(self, individuals, k, objective_index):
        return sorted(
            individuals, key=lambda x: x.fitness.values[objective_index], reverse=True
        )[:k]
