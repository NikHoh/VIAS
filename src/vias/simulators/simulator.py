...
class Simulator(ABC):
    """This is an abstract class (~interface) for any Simulator that is implemented
    in the framework. Every simulator
    ...
    def simulate(self, path: Path) -> float:
        """Starts the simulation. The inputs for the simulation can be found in
        self.input. After the simulation
        its outputs should be written into self.quality_criteria. This is then
        returned by the method."""
        raise NotImplementedError(
            "Users must define simulate_traffic to use this base class."
        )
    ...
    def load_inputs(self, inputs_str: list[str], input_data_folder: str):
        raise NotImplementedError(
            "Users must define load_inputs_and_init to be implemented in the base "
            "class."
        )
    ...
    def init(self):
        raise NotImplementedError(
            "Users must define init to be implemented in the base class."
        )
