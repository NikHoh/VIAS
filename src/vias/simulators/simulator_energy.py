...
c_climb = (
    ...
    * g
    * math.sqrt(
        (2 * average_mass * g / (rho_air * number_rotors * A_rotor)) + v_climb**2
    )
    + 3 / 2 * average_mass * g * v_climb
)
...
def get_velocities_world_fixed(
    path: Path, T: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    """Calculates roll and pitch velocities from the perspective of a world fixed
    coordinate frame"""
...
class SimulatorEnergy(Simulator, GraphBased):
    def __init__(self, **kwargs):
        ...
        GraphBased.__init__(
            self, **kwargs, directed=True
        )  # do not rely on super().__init__(
        # **kwargs) here as kwargs get lost in MRO

        self.cost_z_dir = self.z_dist * 10.0
        self.cost_z_dir_way_back = self.z_dist * 15.0
        self.cost_diag_in_xz_plane = math.sqrt(
            self.x_dist**2 + (self.z_dist * 10.0) ** 2
        )
        self.cost_diag_in_xz_plane_way_back = math.sqrt(
            self.x_dist**2 + (self.z_dist * 15.0) ** 2
        )
        self.cost_diag_in_yz_plane = math.sqrt(
            self.y_dist**2 + (self.z_dist * 10.0) ** 2
        )
        self.cost_diag_in_yz_plane_way_back = math.sqrt(
            self.y_dist**2 + (self.z_dist * 15.0) ** 2
        )
        self.cost_diag_in_xyz = math.sqrt(self.xy_dist**2 + (self.z_dist * 10.0) ** 2)
        self.cost_diag_in_xyz_way_back = math.sqrt(
            self.xy_dist**2 + (self.z_dist * 15.0) ** 2
        )
    ...
    def simulate(self, path: Path) -> float:
        ...
        for time_id, dt in enumerate(T):
            if dh[time_id] > 0:
                ...
                E_climb += (
                    ...
                    * g
                    * math.sqrt(
                        (2 * average_mass * g / (rho_air * number_rotors * A_rotor))
                        + v**2
                    )
                    + 3 / 2 * average_mass * g * v
                ) * dt
            else:
        ...
        for idx, (v_x, v_y) in enumerate(zip(V_x, V_y, strict=False)):
            if v_x == 0:
                theta = 0.0
            else:
                theta = -math.copysign(1, v_x) * math.atan(
                    C_theta * (v_x**2 + v_y**2) / math.sqrt(1 + v_y**2 / (v_x**2))
                )  # formula red box 1
            if theta == 0:
                phi = math.copysign(1, v_y) * math.atan(
                    c_drag * v_y**2 / (average_mass * g)
                )
            else:
                phi = math.atan(-v_y / v_x * math.sin(theta))  # formula red box 2
            if theta != 0:
                assert math.copysign(1, v_x) == -math.copysign(1, theta), (
                    f"v_x is {v_x} " f"but " f"theta is " f"{theta}"
                )
            assert math.copysign(1, v_y) == math.copysign(1, phi), (
                f"v_y is {v_y} but " f"phi is {phi}"
            )
            assert theta >= -math.pi / 4 and theta < math.pi / 4, (
                f" pitch angle" f" theta {theta}" f" exceeds bounds"
            )
            assert phi >= -math.pi / 4 and phi < math.pi / 4, (
                f" roll angle theta " f"{phi} exceeds " f"bounds"
            )
            v_s = math.sqrt(
                C_vs / (math.cos(theta) * math.cos(phi))
                + (math.cos(phi) * math.sin(theta) * v_x - math.sin(phi) * v_y) ** 2
            )
            E_curve += (
                average_mass
                * g
                * (v_s + math.cos(phi) * math.sin(theta) * v_x - math.sin(phi) * v_y)
                / (2 * math.cos(theta) * math.cos(phi))
                * T[idx]
            )
