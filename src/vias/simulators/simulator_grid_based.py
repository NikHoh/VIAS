...
class SimulatorGridBased(Simulator, GridBased):
    ...
    def simulate(self, path: Path) -> float:
        ...
        distances = np.concatenate((np.array([0]), np.sqrt(np.sum(np.diff(path.as_array(), axis=0) ** 2, 1))))
        grid_values = grid_map.get_interpolated_values_from_local_coords(path.waypoint_list)
