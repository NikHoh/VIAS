...
from vias.utils.tools import (
    _save_and_close_matplotlib,
    _save_and_close_plotly,
    get_colors,
)
...
class GridMap(GridGraph):
    """Class that implements a grid grid_map with width and length and resolution
    that can be set. Values of tiles of the
    grid can be set separately or together. Values of tiles can be set or get by
    either the matrix entries of
    the numpy array or by providing x and y cartesian coordinates. The point (0,
    0) of the cartesian frame is fixed
    in the lower left corner of the GridMap."""

    def __init__(self, map_info: MapInfo, dimension=3, init_val=0.0):
        super().__init__(map_info, IGraph(), dimension)
        self.main_layer = 0
        self.grid_tensor: np.ndarray
    ...
    def init_grid_tensor(self, init_val=0.0):
        if self.dimension == 3:
            self.grid_tensor = (
                np.ones(
                    [
                        int(self.y_length / self.y_res),
                        int(self.x_length / self.x_res),
                        int(self.z_length / self.z_res),
                    ]
                )
                * init_val
            )
            ...
            raise AssertionError("Unknown dimension")
    ...
    def local_coord_meshgrid(self) -> MeshGrid:
        """Returns a meshgrid (see numpy meshgrid) consisting of three numpy arrays
        xv, yv, and zv.
        Using the indexing scheme from self.grid_tensor, i.e. an ArrayCoord,
        the output is the respective LocalCoord.
        Example: having the row_index=3, the col_index=2, and the lay_index=1,
        the respective local coordinate entries
        can be obtained by local_coord_meshgrid.xv[3], local_coord_meshgrid.yv[2],
        local_coord_meshgrid.zv[1]"""
        x = np.arange(0.0, self.x_length, self.x_res)
        y = np.flip(np.arange(0.0, self.y_length, self.y_res))
        # flipped as local coord frame y-axis points in different direction than
        # array frame row axis
        z = np.arange(0.0, self.z_length, self.z_res)
    ...
    def array_coord_meshgrid(self) -> MeshGrid:
        """Returns a meshgrid (see numpy meshgrid) that corresponds to the
        self.grid_tensor indices."""
        ...
        xv, yv, zv = np.meshgrid(
            row_indices, column_indices, layer_indices, indexing="ij"
        )
        mesh_grid = MeshGrid(xv.astype("O"), yv.astype("O"), zv.astype("O"))
        return mesh_grid

    @property
    def interpolator_with_extrapolation(self):
        """
        Creates an RegularGridInterpolator (see scipy.interpolate) in the LocalCoord
        frame using the self.grid_tensor
        entries as data.
        """
        if self._interpolator_with_extrapolation is None:
            ...
            if self.dimension == 3:
                z = np.arange(0.0, self.z_length, self.z_res)
                # grid tensor is transposed (x and y swapped) to fit the x y z access
                # order,
                # as y equals the row-axis (first axis) of the array frame
                self._interpolator_with_extrapolation = RegularGridInterpolator(
                    (x, y, z),
    ...
    def interpolator_with_bound_error(self):
        ...
        if self._interpolator_with_bound_error is None:
            ...
            if self.dimension == 3:
                ...
                # order,
                # as y equals the row-axis (first axis) of the array frame
                self._interpolator_with_bound_error = RegularGridInterpolator(
                    (x, y, z),
                    np.transpose(self.grid_tensor, (1, 0, 2)),
                    method="linear",
                    bounds_error=True,
    ...
    def _get_values_from_local_coords(
    ...
    def get_interpolated_values_from_local_coords(
        self, local_coords: list[LocalCoord]
    ) -> np.ndarray:
        """In out of bound this function extrapolates."""
        row_vectors = np.vstack(
            [local_coord.as_array() for local_coord in local_coords]
        )
        interpolator = self.interpolator_with_extrapolation
        if self.dimension == 2:
            assert row_vectors[:, -1].any() == 0.0, (
                "For assessing values in two-dimensional " "array, set z = 0"
            )
    ...
    def get_value_from_local_coord(self, local_coord: LocalCoord) -> float:
        """In out of bound this funtion falls back to get_value_from_array_coord to
        be able to get a value at the edges of the operation space."""
        try:
            interpolator = self.interpolator_with_bound_error
            if self.dimension == 2:
                assert local_coord.z == 0.0, (
                    "For assessing values in " "two-dimensional array, set z = 0"
                )
                return interpolator(local_coord.as_array()[0:2]).item()
            elif self.dimension == 3:
                return interpolator(local_coord.as_array()).item()
        except ValueError:  # out of bound error from interpolation near grid bounds,
            # fal back to get_value from array coord
            return self._get_value_from_local_coord(local_coord)
        raise AssertionError("Unexpected failure")

    def get_values_from_array_coords(
        self, array_coords: list[ArrayCoord]
    ) -> np.ndarray:
        row_vectors = np.vstack(
            [array_coord.as_array() for array_coord in array_coords]
        )
    ...
    def get_value_from_array_coord(self, array_coord: ArrayCoord) -> float:
        return self.grid_tensor[
            array_coord.row, array_coord.col, array_coord.lay
        ].item()
    ...
    def set_from_array(self, input_array: ndarray):
        ...
        if self.dimension <= 3:
            assert (
                self.grid_tensor.shape[0] == input_array.shape[0]
            ), "Grid dimensions do not match."
            assert (
                self.grid_tensor.shape[1] == input_array.shape[1]
            ), "Grid dimensions do not match."
        elif self.dimension == 3:
            assert (
                self.grid_tensor.shape[2] == input_array.shape[2]
            ), "Grid dimensions do not match."
        if self.dimension == 3:
    ...
    def add_from_array(self, input_array: ndarray, operation_type="add"):
        ...
        assert self.grid_tensor.shape == input_array.shape, (
        ...
        if operation_type == "add":
            ...
            raise AssertionError("Operator not known.")
    ...
    def plot_layer_flat(
        ...
        layer=None,
        colormap="cividis",
        norm_min_max: tuple[float, float] | None = None,
        title=None,
        savepath="",
        colbar=False,
        circles=None,
        ...
        if config.suppress_grid_image_plot and config.suppress_grid_image_save:
            ...
            if plot_lib == "matplotlib":
                self._plot_layer_flat_matplotlib(
                    layer=layer,
                    colormap=colormap,
    ...
    def _plot_layer_flat_plotly(
        ...
        fig.add_trace(
            go.Heatmap(
                x=np.arange(0.0, self.x_length, self.x_res),
                y=np.arange(0.0, self.y_length, self.y_res),
        ...
        if circles is not None:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(
                        size=self.x_length / 100, color="gray", symbol="circle"
                    ),
                    name="Circles",
                )
            )
    ...
    def _plot_layer_flat_matplotlib(
        self,
        layer=None,
        colormap="cividis",
        norm_min_max: tuple[float, float] | None = None,
    ...
    def save_layer_landscape_animation(
        self, layer=None, colormap="cividis", title=None, savepath=""
    ):
        ...
        def init():
            self.__plot_layer_landscape_matplotlib(
                fig=fig, ax=ax, layer=layer, colormap=colormap
            )
        ...
        ani.save(
            os.path.join(savepath, f"{self.name}_layer_{layer}.mp4"),
            writer="ffmpeg",
            fps=30,
        )

    def _plot_layer_landscape_matplotlib(
        self,
        layer=None,
        colormap="cividis",
        title=None,
    ...
    def _plot_layer_landscape_plotly(
        self,
        layer=None,
        colormap="cividis",
        title=None,
    ...
    def plot_layer_landscape(
        ...
        if config.suppress_grid_image_plot and config.suppress_grid_image_save:
            ...
            if plot_lib == "matplotlib":
                self._plot_layer_landscape_matplotlib(
                    layer=layer,
                    colormap=colormap,
                    title=title,
                    savepath=savepath,
                ...
                self._plot_layer_landscape_plotly(
                    layer=layer,
                    colormap=colormap,
                    title=title,
                    savepath=savepath,
    ...
    def __plot_layer_landscape_matplotlib(
        self, fig=None, ax=None, layer=None, colormap="cividis", z_axis_title=None
    ):
        if layer is None:
            layer = self.main_layer
        if fig is None:
    ...
    def plot_volume(self, save_path="", title=""):
        ...
        fig = go.Figure(
            data=go.Volume(
                ...
                y=yv.flatten(),
                z=zv.flatten(),
                value=self.grid_tensor.flatten(),
                # Flatten the 3D array for the plot
    ...
    def plot_slices(
        ...
        for layer in range(self.shape[2]):
            savepath = os.path.join(
                map_specific_folder, f"{self.name}_layer{str(layer).zfill(4)}"
            )
            self.plot_layer_flat(
                layer=layer,
                colormap=colormap,
                norm_min_max=(min_val, max_val),
                title=title,
                savepath=savepath,
                colbar=colbar,
                circles=circles,
    ...
    def _plot_paths_matplotlib(
        ...
        for idx, path in enumerate(paths):
            ...
            ax.plot(
                ...
                linestyle=linestyle,
                markersize=2,
            )  # will be shown in the plot() function
        if savepath != "":
            savepath = os.path.join(
                savepath, f"{prefix}opt_path_on_{self.name}{suffix}"
            )
        ...
        for idx, path in enumerate(paths):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=vec_y,
                    mode="markers+lines",
                    # Show both markers and lines
                    marker=dict(
                        color=colors[idx % len(colors)],
                        # Color from the colors list
                        size=2,  # Size of the markers
                        symbol=markerstyle,
                        # Marker style (you can customize this)
                    ),
                    line=dict(
                        color=colors[idx % len(colors)],
                        # Line color (can be different or the
                        # same as markers)
                        dash=linestyle,
                        # Line style (e.g., 'solid', 'dash',
                        # 'dot')
                    ),
                )
            )
    ...
    def plot_paths(
        ...
        """Plots a grid grid_map and overlays it with the plot of a path given by its
        coordinates vec_x and vec_y."""
        config = get_config()
        if config.suppress_grid_image_plot and config.suppress_grid_image_save:
            return
        else:
            if plot_lib == "matplotlib":
                self._plot_paths_matplotlib(
                    paths=paths,
                    savepath=savepath,
                    suffix=suffix,
                    title=title,
    ...
    def plot_3D_paths(
        ...
        """Plots a 3D grid grid_map and overlays it with the plot of a path given by
        its coordinates vec_x and vec_y."""
        config = get_config()
        if config.suppress_grid_image_plot and config.suppress_grid_image_save:
            return
        else:
            if plot_lib == "matplotlib":
                self._plot_3D_paths_matplotlib(
                    paths=paths, savepath=savepath, suffix=suffix, prefix=prefix
                )
            elif plot_lib == "plotly":
                self._plot_3D_paths_plotly(
                    paths=paths, savepath=savepath, suffix=suffix, prefix=prefix
                )
            else:
                raise AssertionError("Unknown plot_lib")

    def _plot_3D_paths_matplotlib(
        self, paths: list[Path] | Path, savepath="", suffix="", prefix=""
    ):
    ...
    def export_lon_lat_height_pd_pkl(self, save_path: str, save_name=None):
        ...
        data = {"lon": longitudes, "lat": latitutes, "height": heights, "value": values}
