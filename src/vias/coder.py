...
class Coder:
    """Class that manages the transformation from a set of vertiport positions to a
    Nurbswork representation (encode)
    the transformation from an individual's gene code representation back to a
    Nurbswork (decode).
    If there is a link (NURBS curve) between a set of points is determined by the
    parameter "link_mask" and can not
    ...
    def encode(self, path: Path) -> np.ndarray:
        """
        Calculates a nurbswork blueprint (nurbswork without links, just nodes) from
        the vertiport positions. Links_mask
        is a binary string, that indicates if there is a link between a pair of
        vport_positions. It is usually
        calculated and saved by the GA Optimizer and can be set in the config.yaml.
        """

        control_points = path.nurbs_curve.ctrlpts[
            1:-1
        ]  # leave out fixed control points
        ...
        if self.use_z_component and self.adaptive_weights:
            ...
            chunked_control_points = chunked_control_points[
                :, 0:2
            ]  # leave out z-column
            chunked_individual = np.hstack((chunked_control_points, weights))
            individual = chunked_individual.reshape(1, -1)
    ...
    def decode(self, individual: np.ndarray) -> Path:
        ...
        If the weights are adaptive (adaptive_weights = True) and the z-component is
        used (use_z_component = True), then it
        looks like the individual looks like
        [x_1, y_1, z_1, w_1, x_2, y_2, z_2, w_2, ...].
        If the inner weights aren't considered (adaptive_weights = False) the
        individual looks like
        [x_1, y_1, z_1, x_2, y_2, z_2, ...].
        if additionally the z-component is not considered (use_z_component = False)
        the individual looks like
        [x_1, y_1, x_2, y_2, ...].
        """

        assert len(individual) == self.size_individual, (
            "Expected individual size " "does not match."
        )
        ...
        control_points = [
            LocalCoord(*col_vec)
            for col_vec in np.vstack((cp_x_vec, cp_y_vec, cp_z_vec)).T
        ]

        path = self.path_factory.generate_path_from_variable_control_points(
            control_points, weight_vec
        )

        return path

    def get_bounds(self):
        """Calculates the bounds for the problem (position constraints for control
        points)"""
        scenario = Scenario()
        if self.use_z_component and self.adaptive_weights:
            low_gene_piece = [0.0, 0.0, 0.0, 0.0]
            up_gene_piece = [
                float(scenario.x_length),
                float(scenario.y_length),
                float(scenario.z_length),
                math.inf,
            ]
        elif self.use_z_component:
