...
from src.vias.constraint_checkers.constraint_checker import Constraint, ConstraintChecker
...
class ConstraintCheckerStaticObstacleCollision(ConstraintChecker, GridBased):
    def __init__(self, **kwargs):
        ...
        GridBased.__init__(self, **kwargs)  # do not rely on super().__init__(**kwargs) here as kwargs get lost in MRO
    ...
    def load_inputs(self, inputs_str: List[str], input_data_folder: str):
        for input_str in inputs_str:
            ...
            if input_str not in self.scenario.grid_maps.keys():
                grid_map = load_grid_graph(input_data_folder, self.scenario.scenario_info, input_str)
    ...
    def init(self):
        ...
        obstacle_distance_transform = get_distances_to_nonzero_cells(inverse_obstacle_array)
        self.processing["maximum_value"] = np.max(obstacle_distance_transform) * np.size(obstacle_distance_transform)
    ...
    def check_constraint(self, path: Path) -> Tuple[float, Constraint]:
        ...
        constraint_value = np.sum(
            self.processing["obstacle_distance_map"].get_interpolated_values_from_local_coords(path.waypoint_list))

        if constraint_value > 0:
            punishment = self.punishment_factor * math.exp(constraint_value / self.processing["maximum_value"])
        else:
            punishment = 0

        constraint = Constraint('static_obstacle_collision', constraint_value, True, constraint_value <= 0)
