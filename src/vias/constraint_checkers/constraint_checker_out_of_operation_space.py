...
class ConstraintCheckerOutOfOperationSpace(ConstraintChecker, NonGraphBased):
    def __init__(self, **kwargs):
        ...
        # lost in MRO
        NonGraphBased.__init__(
            self
        )  # do not rely on super().__init__(**kwargs) here as kwargs get
        # lost in MRO
    ...
    def check_constraint(self, path: Path) -> tuple[float, Constraint]:
        assert path.waypoint_list is not None
        maximum_value = (
            3 * 100.0 * len(path.waypoint_list)
        )  # in all three dimensions all waypoints lie 100m out of bound
        ...
        upper_bound_array = (
            np.full(
                path.as_array().shape,
                [
                    self.scenario.x_length,
                    self.scenario.y_length,
                    self.scenario.z_length,
                ],
            )
            - path.as_array()
        )
        ...
        constraint = Constraint(
            "path_out_of_operation_space", constraint_value, True, constraint_value <= 0
        )
