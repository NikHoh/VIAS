...
class ConstraintCheckerMinimumFlightHeight(ConstraintChecker, GridBased):
    ...
    def check_constraint(self, path: Path) -> Tuple[float, Constraint]:
        num_variable_control_points = PathFactory().num_variable_control_points  # cannot be calculated in init as num of control points can change in preprocessor
        ...
        constraint = Constraint('minimum_flight_height', constraint_value, False, constraint_value <= 0)
