...
class MapCreator(metaclass=abc.ABCMeta):
    """This is an abstract class (~interface) for any grid_map creator that is
    implemented in the framework. Every grid_map creator
    ...
    def create_map(self, map_name) -> GridMap:
        """Creates a grid_map. The inputs for the grid_map creation can be found
        in self.input. After the ceration the grid_map is returned.
        """
        raise NotImplementedError(
            "Users must define create_map() to use this base class."
        )
