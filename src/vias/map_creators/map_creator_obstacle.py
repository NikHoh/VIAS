...
class MapCreatorObstacle(MapCreator):
    def create_map(self, map_name) -> GridMap:
        ...
        console.log(
            f"Map {obstacle_map.name} is being rendered. This may take a while ..."
        )
        ...
        for index, height in np.ndenumerate(buildings_map.grid_array):
            ...
            obstacle_map.grid_tensor[index[0], index[1], 0:lay_idx] = (
                1.0
                # set all
                # elements until height index to 1.0
            )
        ...
        obstacle_map.grid_tensor = binary_dilation(
            obstacle_map.grid_tensor,
            structure=np.ones((structure_y_dim, structure_x_dim, structure_z_dim)),
            iterations=1,
        )
        obstacle_map.grid_tensor = obstacle_map.grid_tensor.astype(float)
